defmodule SeatyReservationWeb.ReservationController do
  ...
  def create(conn, %{"reservation" => reservation_params}) do
    ...
    cond do
      ...
      true ->
        ...
        reservations_seats =
          if reservation_params["seats"] == "" do
            0
          else
            String.to_integer(reservation_params["seats"])
          end
        ...
        if event.total_seats >= number_of_reservations + reservations_seats do
          case Reservations.create_reservation(reservation_params) do
            {:ok, reservation} ->
              deliver(ReservationEmail.confirmation(reservation, event))
          ...
          |> put_flash(
            :error,
            gettext("Zu wenig verfügbare Plätze. Bitte wählen Sie eine andere Aufführung.")
          )
          |> redirect(to: ~p"/reservations/new")
        end
    end
  ...
  def cancel(conn, %{"id" => id}) do
    ...
    case Reservations.update_reservation(reservation, %{
           "seats" => 0,
      ...
      {:ok, reservation} ->
        ...
        event = Events.get_event!(reservation.event_id)
        deliver(ReservationEmail.cancellation(reservation, event))

        conn
        |> put_flash(
          :info,
          "Reservation #{reservation.code} cancelled successfully and cancellation email sent."
        )
        |> redirect(to: ~p"/reservations/#{id}/edit")
  ...
  defp deliver(email) do
    case Mailer.deliver(email) do
      {:ok, _} -> :ok
      {:error, reason} -> Logger.error("Failed to deliver email: #{inspect(reason)}")
    end
  end
